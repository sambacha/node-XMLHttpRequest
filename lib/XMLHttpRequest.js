...
exports.XMLHttpRequest = function () {
  ...
  this.send = function (data) {
    ...
    if (settings.async) {
      ...
      const responseHandler = function responseHandler (resp) {
        ...
        if (encoding === 'utf8') {
          response.on('data', function (chunk) {
            // console.log('unxhr data utf8', chunk.length)
          ...
          response.on('end', function () {
            // console.log('unxhr sendFlag', sendFlag)
            if (sendFlag) {
          ...
          response.on('data', function (chunk) {
            // console.log('unxhr data binary', chunk.length)
            // Make sure there's some data
            if (chunk) {
              self.responseBinary.push(chunk)
            }
            // Don't emit state changes if the connection has been aborted.
            if (sendFlag) {
              setState(self.LOADING)
          ...
          response.on('end', function () {
            ...
            self.response = Buffer.concat(self.responseBinary).buffer
            if (sendFlag) {
              // Discard the end event if the connection has been aborted
              setState(self.DONE)
              sendFlag = false
      ...
      const output = require('child_process').execSync(`"${process.argv[0]}" "${scriptPath}" \
--ssl="${ssl}" \
--encoding="${encoding}" \
